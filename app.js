...
function rerenderDetectionResults() {
  ...
  // Draw depth heatmap jika toggle aktif (sebelum bounding boxes)
  if (showHeatmap && lastDetections.length > 0) {
    drawDepthHeatmap(ctx, lastDetections, resultCanvas.width, resultCanvas.height, lastDetectionCanvas);
  }
...
function displayDetectionResults(originalCanvas, detections) {
  ...
  if (showHeatmap && detections.length > 0) {
    drawDepthHeatmap(ctx, detections, resultCanvas.width, resultCanvas.height, originalCanvas);
  }
